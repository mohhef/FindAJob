...
class RepController
{
    ...
    {
        $conn = connDB();
        $manager = $_COOKIE['employer_username'];
        if ($stmt = $conn->prepare("SELECT category, preferred_method, company_name FROM employer WHERE user_name = ?")) {
            ...
            if ($stmt = $conn->prepare("INSERT INTO all_user(user_name, email, balance, password) VALUE (?, ?, 0, ?)")) {
                ...
                if ($stmt->affected_rows > 0) {
                    ...
                    if ($stmt = $conn->prepare("INSERT INTO representatives(rep_user_name, employer_user_name) VALUE (?, ?)")) {
                        ...
                        if ($stmt->affected_rows > 0) {
                            $stmt->close();
                            if ($stmt = $conn->prepare("INSERT INTO employer(user_name, category, preferred_method, company_name) VALUE (?, ?, ?, ?)")) {
                                $stmt->bind_param("ssss", $username, $arr[0][0], $arr[0][1], $arr[0][2]);
                                $stmt->execute();
                                if ($stmt->affected_rows > 0) {
                                    return true;
